...
int dsc1000_sendcmd(Camera *camera, uint8_t cmd, void *data, int size) {
...
int dsc1000_retrcmd(Camera *camera) {
...
/*      Make sense in debug only. Done on gp_port level.
        if (0 < s)
                dsc1000_dumpmem(camera->pl->buf, s);
*/
...
int dsc1000_setbaudrate(Camera *camera, int speed) {
	...
	GPPortSettings settings;
        ...
        if (dsc1000_sendcmd(camera, DSC1_CMD_SET_BAUD, &s_bps, 1) != GP_OK)
                return GP_ERROR;
...
int dsc1000_getmodel(Camera *camera) {

        static const char       response[3] = { 'D', 'S', 'C' };

        DEBUG_PRINT_MEDIUM(("Getting camera model."));

        if (dsc1000_sendcmd(camera, DSC1_CMD_GET_MODEL, NULL, 0) != GP_OK)
                return GP_ERROR;

        if (dsc1000_retrcmd(camera) != DSC1_RSP_MODEL ||
...
void dsc1000_dumpmem(void *buf, int size) {
...
const char *dsc1000_strerror(int error) {
...
char *dsc1000_msgprintf(char *format, ...) {
...
void dsc1000_errorprint(int error, char *file, int line) {

	GP_DEBUG(
		"%s:%u: return code: %i, errno: %i, %s",
			file, line, error, errno, dsc1000_strerror(error));
}
