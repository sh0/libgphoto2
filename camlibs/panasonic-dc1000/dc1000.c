...
/* dsc1000_connect - try hand shake with camera and establish connection */

static int dsc1000_connect(Camera *camera, int speed) {

	uint8_t	data = 0;

	DEBUG_PRINT_MEDIUM(("Connecting a camera."));

	if (dsc1000_setbaudrate(camera, speed) != GP_OK)
		return GP_ERROR;

	if (dsc1000_getmodel(camera) != DSC1)
		RETURN_ERROR(EDSCBADDSC);
		/* bad camera model */

	dsc1000_sendcmd(camera, DSC1_CMD_CONNECT, &data, 1);

	if (dsc1000_retrcmd(camera) != DSC1_RSP_OK)
...
/* dsc1000_disconnect - reset camera, free buffers and close files */

static int dsc1000_disconnect(Camera *camera) {

	DEBUG_PRINT_MEDIUM(("Disconnecting the camera."));

	if (dsc1000_sendcmd(camera, DSC1_CMD_RESET, NULL, 0) != GP_OK)
		return GP_ERROR;
...
/* dsc1000_getindex - retrieve the number of images stored in camera memory */

static int dsc1000_getindex(Camera *camera) {

	int	result = GP_ERROR;

	DEBUG_PRINT_MEDIUM(("Retrieving the number of images."));

	if (dsc1000_sendcmd(camera, DSC1_CMD_GET_INDEX, NULL, 0) != GP_OK)
		return GP_ERROR;

	if (dsc1000_retrcmd(camera) == DSC1_RSP_INDEX)
...
/* dsc1000_delete - delete image #index from camera memory */

static int dsc1000_delete(Camera *camera, uint8_t index) {
	...
	if (dsc1000_sendcmd(camera, DSC1_CMD_DELETE, &index, 1) != GP_OK)
		return GP_ERROR;
...
/* dsc1000_selectimage - select image to download, return its size */

static int dsc1000_selectimage(Camera *camera, uint8_t index)
{
        int	size = 0;
	...
	if (dsc1000_sendcmd(camera, DSC1_CMD_SELECT, &index, 1) != GP_OK)
		return GP_ERROR;

	if (dsc1000_retrcmd(camera) != DSC1_RSP_IMGSIZE)
		RETURN_ERROR(EDSCBADRSP);
		/* bad response */
...
static int dsc1000_readimageblock(Camera *camera, int block, char *buffer) {
	...
	if (dsc1000_sendcmd(camera, DSC1_CMD_GET_DATA, buf, 2) != GP_OK)
		return GP_ERROR;

	if (dsc1000_retrcmd(camera) != DSC1_RSP_DATA)
		RETURN_ERROR(EDSCBADRSP);
		/* bad response */
...
/* dsc1000_readimage - read #index image or thumbnail and return its contents */
#if 0
static char *dsc1000_readimage(Camera *camera, int index, int *size) {
	...
	if ((*size = dsc1000_selectimage(camera, index)) < 0)
		return NULL;
	...
	for (i = 0, s = 0; s < *size; i++) {
		if ((rsize = dsc1000_readimageblock(camera, i, &buffer[s])) == GP_ERROR) {
...
/* dsc1000_setimageres - set image resolution based on its size, used on upload to camera */

static int dsc1000_setimageres(Camera *camera, int size) {

	dsc1000_quality_t	res;
	...
	if (dsc1000_sendcmd(camera, DSC1_CMD_SET_RES, &res, 1) != GP_OK)
		return GP_ERROR;
...
static int dsc1000_writeimageblock(Camera *camera, int block, char *buffer, int size) {

	DEBUG_PRINT_MEDIUM(("Writing image block: %i", block));

	dsc1000_sendcmd(camera, DSC1_CMD_SEND_DATA, buffer, size);
...
#if 0
static int dsc1000_writeimage(Camera *camera, char *buffer, int size)
{
	int	blocks, blocksize, i;

	DEBUG_PRINT_MEDIUM(("Writing an image of size: %i.", size));

	if ((dsc1000_setimageres(camera, size)) != GP_OK)
		return GP_ERROR;
	...
	for (i = 0; i < blocks; i++) {
		...
		if (dsc1000_writeimageblock(camera, i, &buffer[i*DSC_BLOCKSIZE], blocksize) != GP_OK) {
			DEBUG_PRINT_MEDIUM(("Error during image transfer."));
			return GP_ERROR;
		}
	}
...
/* dsc1000_preview - show selected image on camera's LCD - is it supported? */

#if 0
static int dsc1000_preview(Camera *camera, int index)
{
	if (index < 1)
		RETURN_ERROR(EDSCBADNUM);
		/* bad image number */

	if (dsc1000_sendcmd(camera, DSC1_CMD_PREVIEW, &index, 1) != GP_OK)
		return GP_ERROR;
			   ...
			   CameraList *list, void *data, GPContext *context)
{
	...
	if ((count = dsc1000_getindex (camera)) == GP_ERROR)
		return GP_ERROR;
			  ...
			  CameraFile *file, void *data, GPContext *context)
{
	...
	if ((size = dsc1000_selectimage(camera, index + 1)) < 0)
		return (GP_ERROR);
	...
	for (i = 0, s = 0; s < size; i++) {
		if ((rsize = dsc1000_readimageblock(camera, i, NULL)) == GP_ERROR)
			return GP_ERROR;
		s += rsize;
...
/*	We can not figure out file type, at least by now.
	...
	if (strcmp(file->type, "image/jpg") != 0) {
		dsc1000_print_message(camera, "JPEG image format allowed only.");
		return GP_ERROR;
	}
*/
	...
	result = dsc1000_setimageres (camera, size);
	if (result != GP_OK)
		return (result);
	...
	for (i = 0; i < blocks; i++) {
		...
		if (DSC_BLOCKSIZE < blocksize)
			blocksize = DSC_BLOCKSIZE;
		result = dsc1000_writeimageblock (camera, i,
			     ...
			     GPContext *context)
{
	...
	return dsc1000_delete(camera, index);
}
...
static int camera_init (Camera *camera, GPContext *context) {
	...
	ret = dsc1000_connect(camera, selected_speed);
	if (ret < 0) {
		free (camera->pl->buf);
		free (camera->pl);
		camera->pl = NULL;
...
CameraLibrary camera_panasonic_dc1000_library = {
    .id = "panasonic-dc1000",
