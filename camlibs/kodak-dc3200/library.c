...
int dc3200_get_data(Camera *camera, unsigned char **data, unsigned long *data_len, int command, const char *folder, const char *filename)
{
	...
	do {
		...
		case 0xC1:
			...
			*data_len = dc3200_bytes_to_l(resp[26], resp[27], resp[28], resp[29]);
			num_left = 0;
			data_start_pos = 31;
			*data_len -= 1;
		...
		case 0x41:
			...
			/* get the total list length from the data header */
			*data_len = dc3200_bytes_to_l(resp[34], resp[35], resp[36], resp[37]);
			num_left = dc3200_bytes_to_l(resp[12], resp[13], resp[14], resp[15]);
			data_start_pos = 39;
			*data_len -= 1;
...
#ifdef DEBUG
	dc3200_dump_buffer(buff, buff_len, "s", 16);
#endif
...
#ifdef DEBUG
	dc3200_dump_buffer(buff, num_read, "r", 16);
#endif
 ...
 *
 */
int dc3200_dump_buffer(unsigned char * buffer, int len, char * title, int bytesperline)
{
	char spacer[80];
	int i;
...
unsigned long dc3200_bytes_to_l(int a, int b, int c, int d)
{
	unsigned long res = a;
