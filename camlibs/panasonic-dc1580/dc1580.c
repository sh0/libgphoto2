...
static int dsc2_connect(Camera *camera, int speed) {
        ...
        if (dsc1580_setbaudrate(camera, speed) != GP_OK)
                return GP_ERROR;

        if (dsc1580_getmodel(camera) != DSC2)
...
/*      We can not figure out file type, at least by now. (? curious, mime type -Marcus)
        ...
        if (strcmp(file->type, "image/jpg") != 0) {
                dsc1580_print_message(camera, "JPEG image format allowed only.");
                return GP_ERROR;
        }
*/
...
CameraLibrary camera_panasonic_dc1580_library = {
    .id = "panasonic-dc1580",
