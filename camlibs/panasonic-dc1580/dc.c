...
int dsc1580_sendcmd(Camera *camera, uint8_t cmd, void *data, int size) {
...
int dsc1580_retrcmd(Camera *camera) {
...
/*      Make sense in debug only. Done on gp_port level.
        if (0 < s)
                dsc1580_dumpmem(camera->pl->buf, s);
*/
...
int dsc1580_setbaudrate(Camera *camera, int speed) {
	...
	GPPortSettings settings;
        ...
        if (dsc1580_sendcmd(camera, DSC1_CMD_SET_BAUD, &s_bps, 1) != GP_OK)
                return GP_ERROR;

        if (dsc1580_retrcmd(camera) != DSC1_RSP_OK)
                RETURN_ERROR(EDSCBADRSP);
                /* bad response */
...
int dsc1580_getmodel(Camera *camera) {

        static const char       response[3] = { 'D', 'S', 'C' };

        DEBUG_PRINT_MEDIUM(("Getting camera model."));

        if (dsc1580_sendcmd(camera, DSC1_CMD_GET_MODEL, NULL, 0) != GP_OK)
                return GP_ERROR;

        if (dsc1580_retrcmd(camera) != DSC1_RSP_MODEL ||
...
void dsc1580_dumpmem(void *buf, int size) {
...
const char *dsc1580_strerror(int error) {
...
char *dsc1580_msgprintf(char *format, ...) {
...
void dsc1580_errorprint(int error, char *file, int line) {

	GP_DEBUG(
		"%s:%u: return code: %i, errno: %i, %s",
			file, line, error, errno, dsc1580_strerror(error));
}
