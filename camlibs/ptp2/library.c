...
camera_trigger_canon_eos_capture (Camera *camera, GPContext *context)
{
    ...
    if (ptp_operation_issupported(params, PTP_OC_CANON_EOS_RemoteReleaseOn)) {
        ...
        case 7:
            ...
            return GP_ERROR_NO_MEMORY;
        case 8:
            gp_context_error (context, _("Canon EOS Capture failed to release: Card read-only?"));
            return GP_ERROR_NO_MEMORY;
        default:
...
#define APPEND_TXT( ... ) txt += snprintf (txt, SPACE_LEFT, __VA_ARGS__)
    ...
    if (ptp_operation_issupported(params,PTP_OC_GetStorageIDs) &&
        ...
        for (i=0; i<params->storageids.n; i++) {
            ...
            case PTP_FST_GenericHierarchical:
                s = _("Generic Hierarchical");
                break;
            case PTP_FST_DCF:
                s = _("Digital Camera Layout (DCIM)");
                break;
            default:
            ...
            case PTP_AC_ReadOnly_with_Object_Deletion:
                s = _("Read Only with Object deletion");
                break;
            default:
    ...
    for (i=0; i<pdi.DevicePropertiesSupported_len; i++) {
        ...
        if (ret == PTP_RC_OK) {
            switch (dpd.GetSet) {
            case PTP_DPGS_Get:
                APPEND_TXT ("(%s) ", N_("read only"));
                break;
            case PTP_DPGS_GetSet:
                APPEND_TXT ("(%s) ", N_("readwrite"));
                break;
            default:
                APPEND_TXT ("(%s) ", N_("Unknown"));
            }
...
ptp_mtp_render_metadata (
    ...
    if (mprops) { /* use the fast method, without device access since cached.*/
        ...
        for (j=0; j<ob->nrofmtpprops; j++) {
            ...
            switch (xpl->datatype) {
            default:
                sprintf (text, "Unknown type %d", xpl->datatype);
                break;
            case PTP_DTC_STR:
    ...
    for (j=0; j<propcnt; j++) {
        ...
        if (ret == PTP_RC_OK) {
            ...
            if (ret != PTP_RC_OK) {
                ...
                switch (opd.DataType) {
                default:
                    sprintf (text, "Unknown type %d", opd.DataType);
                    break;
                case PTP_DTC_STR:
