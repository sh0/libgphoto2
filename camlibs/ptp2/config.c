...
_get_INT(CONFIG_GET_ARGS) {
    ...
    if (dpd->FormFlag == PTP_DPFF_Enumeration) {
        ...
        for (i=0; i<dpd->FORM.Enum.NumberOfValues; i++) {
            switch (dpd->DataType) {
            case PTP_DTC_UINT32:
                sprintf (value, "%u", dpd->FORM.Enum.SupportedValue[i].u32 );
                break;
            case PTP_DTC_INT32:
                sprintf (value, "%d", dpd->FORM.Enum.SupportedValue[i].i32 );
                break;
            case PTP_DTC_UINT16:
                sprintf (value, "%u", dpd->FORM.Enum.SupportedValue[i].u16 );
                break;
            case PTP_DTC_INT16:
                sprintf (value, "%d", dpd->FORM.Enum.SupportedValue[i].i16 );
                break;
            case PTP_DTC_UINT8:
                sprintf (value, "%u", dpd->FORM.Enum.SupportedValue[i].u8  );
                break;
            case PTP_DTC_INT8:
                sprintf (value, "%d", dpd->FORM.Enum.SupportedValue[i].i8  );
                break;
            default:
                sprintf (value,_("unexpected datatype %i"),dpd->DataType);
                return GP_ERROR;
...
_get_Canon_CameraOutput(CONFIG_GET_ARGS) {
    ...
    for (i=0; i<dpd->FORM.Enum.NumberOfValues; i++) {
        ...
        default:
            sprintf(buf,_("Unknown %d"),dpd->FORM.Enum.SupportedValue[i].u8);
            x=buf;
            break;
        case 1:
            x=_("LCD");
...
_put_Canon_CameraOutput(CONFIG_PUT_ARGS) {
    ...
    u = -1;
    if (!strcmp(value,_("LCD"))) {
        u = 1;
    }
    if (!strcmp(value,_("Video OUT"))) {
        u = 2;
    }
    if (!strcmp(value,_("Off"))) {
        u = 3;
...
_get_Olympus_ISO(CONFIG_GET_ARGS) {
    ...
    for (i=0; i<dpd->FORM.Enum.NumberOfValues; i++) {
        ...
        if (dpd->FORM.Enum.SupportedValue[i].u16 == 0xfffd) {
            strcpy(buf,_("Low"));
        }
        gp_widget_add_choice (*widget,buf);
...
_get_Canon_EOS_BatteryLevel(CONFIG_GET_ARGS) {
    ...
    case 0:
        ...
        break;
    case 1:
        gp_widget_set_value(*widget, _("50%"));
        break;
    case 2:
        gp_widget_set_value(*widget, _("100%"));
        break;
    case 4:
        gp_widget_set_value(*widget, _("75%"));
        break;
    case 5:
        gp_widget_set_value(*widget, _("25%"));
        break;
    default:
    ...
    if (val) {
        ...
        if (have_prop(camera, params->deviceinfo.VendorExtensionID, PTP_DPC_NIKON_LiveViewProhibitCondition)) {
            ...
            if (value.u32) {
                ...
                if (value.u32 & (1<<8)) {
                    gp_context_error (context, _("Liveview cannot start: Battery exhausted"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<17)) {
                    gp_context_error (context, _("Liveview cannot start: Temperature too high"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<9)) {
                    gp_context_error (context, _("Liveview cannot start: TTL error"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<22)) {
                    gp_context_error (context, _("Liveview cannot start: In Mirror-up operation"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<24)) {
                    gp_context_error (context, _("Liveview cannot start: Lens is retracting"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<5)) {
                    gp_context_error (context, _("Liveview cannot start: Minimum aperture warning"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<15)) {
                    gp_context_error (context, _("Liveview cannot start: Processing of shooting operation"));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<2)) {
                    gp_context_error (context, _("Liveview cannot start: Sequence error"));
                    return GP_ERROR;
                }
...
_put_Nikon_Movie(CONFIG_PUT_ARGS)
{
    ...
    if (val) {
        ...
        if (have_prop(camera,PTP_VENDOR_NIKON,PTP_DPC_NIKON_MovRecProhibitCondition)) {
            ...
            if (value.u32) {
                if (value.u32 & (1<<14)) {
                    gp_context_error (context, _("Movie recording cannot start: Camera is not in application mode."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<13)) {
                    gp_context_error (context, _("Movie recording cannot start: Set liveview selector is enabled."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<12)) {
                    gp_context_error (context, _("Movie recording cannot start: In enlarged liewview."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<11)) {
                    gp_context_error (context, _("Movie recording cannot start: Card protected."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<<10)) {
                    gp_context_error (context, _("Movie recording cannot start: Already in movie recording."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<< 9)) {
                    gp_context_error (context, _("Movie recording cannot start: Images / movies not yet record in buffer."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<< 3)) {
                    gp_context_error (context, _("Movie recording cannot start: Card full."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<< 2)) {
                    gp_context_error (context, _("Movie recording cannot start: Card not formatted."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<< 1)) {
                    gp_context_error (context, _("Movie recording cannot start: Card error."));
                    return GP_ERROR;
                }
                if (value.u32 & (1<< 0)) {
                    gp_context_error (context, _("Movie recording cannot start: No card."));
                    return GP_ERROR;
                }
