...
} uw32_t; /* A type for 32-bit integers */
typedef struct
{
    unsigned char c1, c2, c3, c4;
} uw4c_t; /* A type for 4-byte things */
...
traverse_output_tree (PTPParams *params, xmlNodePtr node, PTPContainer *resp) {
    ...
    case PTP_OC_GetDevicePropDesc:
        ...
        /* TODO */
#if 0
    case PTP_OC_OLYMPUS_GetDeviceInfo:
        return parse_9301_tree (next); /* 9301 */
#endif
    case PTP_OC_OLYMPUS_OpenSession:
        return parse_9302_tree (next);
    case PTP_OC_OLYMPUS_GetCameraControlMode:
        return parse_910a_tree (next);
    case PTP_OC_OLYMPUS_GetCameraID:
        return parse_9581_tree (next);
